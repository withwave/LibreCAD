...
void dwgReader::DwgEntityOutput::appendValue(const T &value,
  ...
  if constexpr (std::is_invocable_v<Callback, DRW_Interface &, const T &>) {
    ...
    public:
      ...
      bool replay(dwgReader &reader, DRW_Interface &target) const override {
        bool routed = false;
        if constexpr (std::is_base_of_v<DRW_Entity, T>) {
          // Legacy recovery sweeps can encounter definition-owned entities
          // after their BLOCK scope has closed. Keep them in that definition.
          routed = m_value.eType != DRW::BLOCK &&
              m_value.parentHandle != DRW::NoHandle &&
              reader.expectedBlockEntityOwner == DRW::NoHandle &&
              !reader.ownerlessSpaceWalk &&
              reader.blockRecordmap.count(m_value.parentHandle) != 0;
          if (routed) target.setBlock(static_cast<int>(m_value.parentHandle));
        }
        try {
          std::invoke(m_callback, target, m_value);
        } catch (...) {
          if (routed) target.setBlock(0);
          throw;
        }
        if (routed) target.setBlock(0);
        return true;
...
dwgReader::DwgMappedEntityOutcome dwgReader::deliverPreparedPolylineCommit(
  ...
  try {
    ...
    output.appendValue(pending.entity, &DRW_Interface::addPolyline);
  } catch (...) {
    terminalizePendingPolylineState(handle,
...
dwgReader::DwgMappedEntityOutcome dwgReader::deliverPreparedInsertCommit(
  ...
  try {
    DwgImmediateEntityOutput output(*this, intfa);
    output.appendValue(pending.entity, &DRW_Interface::addInsert);
  } catch (...) {
