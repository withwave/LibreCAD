...
LC_PrintPreviewOptionsWidget::LC_PrintPreviewOptionsWidget():ui(new Ui::LC_PrintPreviewOptionsWidget{}) {
    ...
    connect(printerArea, &QToolButton::clicked, this, [this] {
        if (m_action) m_action->invokePrinterDialog();
    });
    ...
    connect(ui->cbScale->lineEdit(), &QLineEdit::editingFinished, [this] {
        // Merely opening a dialog must not commit the rounded display text.
        if (!ui->cbScale->lineEdit()->isModified()) return;
        ui->cbScale->lineEdit()->setModified(false);
    ...
    connect(ui->cbScale, &QComboBox::currentIndexChanged, [this](const int index) {
        ui->cbScale->blockSignals(true);
        const QVariant value = ui->cbScale->itemData(index);
        if (value.isValid()) scale(QString("%1:1").arg(value.toDouble(), 0, 'g', 17));
        ui->cbScale->blockSignals(false);
    });
