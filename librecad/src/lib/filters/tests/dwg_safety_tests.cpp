...
TEST_CASE("Dimension handles resolve relative style and display-block references",
          "[dwg-read][dimension][handles]") {
    struct DimensionHandles : DRW_DimAligned {
        using DRW_Dimension::parseDwgDimensionHandles;
        std::uint32_t styleHandle() const { return dimStyleH.ref; }
        std::uint32_t displayHandle() const { return blockH.ref; }
    };
    // R2004: null extension dictionary, null layer, relative +1 style,
    // relative -1 anonymous display block. No owner/reactors in this frame.
    std::vector<std::uint8_t> bytes{0x00, 0x00, 0x60, 0x80};
    dwgBuffer buffer(bytes.data(), bytes.size());
    DimensionHandles dimension;
    dimension.handle = 0x100;
    REQUIRE(dimension.parseDwgDimensionHandles(DRW::AC1018, &buffer, bytes.size() * 8));
    CHECK(dimension.styleHandle() == 0x101);
    CHECK(dimension.displayHandle() == 0xff);
}
