...
void RS_Graphic::onLoadingCompleted() {
    ...
    const auto updateLoadedDimensions = [](auto&& self, RS_EntityContainer& container) -> void {
        for (auto* entity : container) {
            if (!entity || entity->isDeleted()) continue;
            if (auto* dimension = dynamic_cast<RS_Dimension*>(entity)) {
                if (!dimension->hasImportedDisplay()) dimension->update();
            } else if (entity->rtti() == RS2::EntityDimLeader) {
                entity->update();
            } else if (entity->isContainer()) {
                self(self, *static_cast<RS_EntityContainer*>(entity));
            }
        }
    };
    updateLoadedDimensions(updateLoadedDimensions, *this);
}
