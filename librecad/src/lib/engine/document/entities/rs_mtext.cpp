...
void RS_MText::update() {
  ...
  std::vector<double> heightScopes;
  ...
  auto closeLine = [&]() {
    ...
    for (decltype(m_data.text.length()) i = 0; i < m_data.text.length(); ++i) {
        ...
        if (remaining.startsWith(QLatin1Char('{'))) {
            heightScopes.push_back(heightScale);
            continue;
        }
        if (remaining.startsWith(QLatin1Char('}')) && !heightScopes.empty()) {
            heightScale = heightScopes.back();
            heightScopes.pop_back();
            continue;
        }
        if (remaining.startsWith(QStringLiteral("\\A")) && remaining.size() >= 4 &&
            remaining.at(2) >= QLatin1Char('0') && remaining.at(2) <= QLatin1Char('2') &&
            remaining.at(3) == QLatin1Char(';')) {
            i += 3;
            continue;
        }
        if (remaining.startsWith(QStringLiteral("\\H"))) {
            const auto end = remaining.indexOf(';');
            if (end > 2) {
                QString value = remaining.mid(2, end - 2);
                const bool relative = value.endsWith(QLatin1Char('x'), Qt::CaseInsensitive);
                if (relative) value.chop(1);
                bool ok = false;
                const double height = value.toDouble(&ok);
                if (ok && std::isfinite(height) && height > 0. && m_data.height > 0.) {
                    const double nextScale = relative ? heightScale * height : height / m_data.height;
                    if (std::isfinite(nextScale) && nextScale > RS_TOLERANCE) {
        ...
        if (remaining.startsWith(R"(\F)") && remaining.indexOf(R"(^\\[Ff]\{[\d\w]*\})") != 0) {
            segments.push_back(
          {LC_BidiSegment::Char, m_data.text.at(i), font, 0.0, {}, {}, heightScale});
            continue;
        }
        ...
        switch (m_data.text.at(i).unicode()) {
            ...
            case 0x20:
                // Space:
                segments.push_back(
          {LC_BidiSegment::Space, QChar(' '), nullptr, spaceWidth * heightScale, {}, {}, heightScale});
                break;
    ...
    case LC_BidiSegment::Char: {
      ...
      if (segFont != nullptr) {
        addLetter(oneLine, seg.codepoint, *segFont, letterSpace,
                  letterPosition, seg.heightScale);
      }
      break;
    }
    case LC_BidiSegment::Space:
 ...
 *
 */
void RS_MText::addLetter(LC_TextLine& oneLine, const QChar letter, RS_Font& font, const RS_Vector& letterSpace, RS_Vector& letterPosition, double heightScale) {
    ...
    const RS_InsertData d(letterText, letterPosition, RS_Vector(heightScale, heightScale), 0.0, 1, 1,
    ...
    double actualWidth = (letterEntity->getMax().x - letterEntity->getMin().x) / heightScale;
    ...
    letterPosition.x += actualWidth * heightScale;

    oneLine.addEntity(letterEntity);

    // next letter position:
    letterPosition += letterSpace * heightScale;
}
