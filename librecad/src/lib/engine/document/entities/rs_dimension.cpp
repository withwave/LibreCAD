...
QString RS_Dimension::getLabel(const bool resolve) {
    ...
    QString ret = m_dimGenericData.text;
    ...
    // No text prints actual measurement:
        ...
        if (rtti() == RS2::EntityDimDiametric) {
            ret.prepend(QChar(0x2300));
        } else if (rtti() == RS2::EntityDimRadial) {
            ret.prepend(QLatin1Char('R'));
        }
    }
    ...
    if (m_dimGenericData.text.isEmpty() && m_dimStyleTransient) {
        const auto* tolerance = m_dimStyleTransient->latteralTolerance();
        if (tolerance->isAppendTolerancesToDimText() &&
            !tolerance->isLimitsGeneratedAsDefaultText() &&
            tolerance->upperToleranceLimit() == tolerance->lowerToleranceLimit()) {
            QString amount = QString::number(tolerance->upperToleranceLimit(), 'f',
                                              std::clamp(tolerance->decimalPlaces(), 0, 16));
            amount = stripZerosLinear(amount, m_dimStyleTransient->zerosSuppression()->toleranceRaw());
            if (getDimDecimalFormatSeparatorChar() == 44) amount.replace('.', ',');
            const double factor = tolerance->heightScaleFactorToDimText();
            if (std::isfinite(factor) && factor > 0.) {
                ret += QStringLiteral("{\\H%1x;±%2}").arg(factor).arg(amount);
            }
        }
    }
