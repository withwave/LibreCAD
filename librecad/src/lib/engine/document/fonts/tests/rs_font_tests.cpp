...
TEST_CASE("standard.lff still falls back to the replacement character for a "
    ...
    // Private-use characters may exist in a system fallback font. Use a
    // Unicode noncharacter to test the unsupported-glyph path deterministically.
    RS_Block *letter = font.findLetter(QString(QChar(char16_t{0xFFFF})));
