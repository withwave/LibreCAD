...
static void drawGraphic(RS_Graphic *graphic, QPrinter &printer, RS_Painter &painter, const PdfPrintParams& params);
...
bool PdfPrintLoop::printOneFileToOnePdf(const QString& inputFile) {
    ...
    drawGraphic(graphic, printer, painter, m_params);
...
int PdfPrintLoop::printManyFilesToOnePdf() {
    ...
    for (const auto &item : contentItems) {
        ...
        drawGraphic(item.graphic, printer, painter, m_params);
...
static void drawGraphic(RS_Graphic* graphic, QPrinter& printer,
                        RS_Painter& painter, const PdfPrintParams& params){
    // A backend can reject the drawing's paper size and select another page.
    // Fit against that actual page, including in multi-document output.
    const QSizeF pageMm = printer.pageLayout().fullRect(QPageLayout::Millimeter).size();
    auto* plot = graphic->getPlotSettings();
    const RS_Vector actualSize = RS_Units::convert(RS_Vector(pageMm.width(), pageMm.height()),
                                                  RS2::Millimeter, graphic->getUnit());
    if (plot->getPaperSize().distanceTo(actualSize) > 0.01) {
        plot->setPaperSize(actualSize);
        if (params.fitToPage) graphic->fitToPage();
        else if (params.centerOnPage) graphic->centerToPage();
    }
    const double printerWidth = printer.width();
    const double printerHeight = printer.height();
    const double printerFx = printer.logicalDpiX() / 25.4;
    const double printerFy = printer.logicalDpiY() / 25.4;
