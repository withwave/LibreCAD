...
TEST_CASE("automatic print preview fits once after loading options",
    ...
    // The preview applies driver/default margins when it loads. Its fit
    // must use that resulting printable area, not the earlier zero margins.
    ...
    CHECK(graphic.getPlotSettings()->getMarginLeftMm() >= 2.5);
...
TEST_CASE("Changing print scale preserves the center of a distant drawing", "[print_preview]") {
    (void)application();
    SettingGuard fixed{RS_SETTINGS, "ActionFilePrintPreview", "PrintScaleFixed"};
    fixed.set(false);
    RS_Graphic graphic;
    graphic.initForNewDocument();
    graphic.addEntity(new RS_Line(&graphic, RS_LineData({15900., -37600.}, {16100., -37300.})));
    graphic.calculateBorders();
    PrintPreviewTestView view;
    view.setDocument(&graphic);
    LC_ActionContext context;
    ...
    auto action = std::make_unique<RS_ActionPrintPreview>(&context);
    action->postCreateInit();
    const RS_Vector center = graphic.getMin() + graphic.getSize() * 0.5;
    const RS_Vector before = center * action->getScale() + graphic.getPaperInsertionBase();
    REQUIRE(action->setScale(action->getScale() * 0.75, false));
    const RS_Vector after = center * action->getScale() + graphic.getPaperInsertionBase();
    CHECK(after.x == Catch::Approx(before.x).margin(1e-8));
    CHECK(after.y == Catch::Approx(before.y).margin(1e-8));
}
