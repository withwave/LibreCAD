...
void RS_ActionPrintPreview::invokeSettingsDialog() const {
    if (m_graphic != nullptr) {
        ...
        if (!isPaperScaleFixed()) fit(); else center();
...
void RS_ActionPrintPreview::invokePrinterDialog() const {
...
bool RS_ActionPrintPreview::setScale(const double newScale, const bool autoZoom) const {
    if (m_graphic != nullptr) {
        ...
        // Preserve the paper position of the actual drawing center, including
        // its distance from the world origin (common in imported DWGs).
        const RS_Vector center = m_graphic->getMin() + m_graphic->getSize() * 0.5;
        pinBase += center * (oldScale - newScale);
